...
void CCustomTitleUI::_HudModelSetting()
{
	...
	if (pHud != NULL)
	{
		...
		pHud->HUD_SetModelData(HUD_CHARACTER_TYPE, -1, eRENDER_UI_TYPE_MAKETITLE, (INDEX)_pNetwork->Get_MyChar_Job());
...
void CCustomTitleUI::zoom_plus()
{
	//CPlayerEntity* pModel = ((CPlayerEntity*)CEntity::GetPlayerEntity(0));
...
void CCustomTitleUI::zoom_minus()
{
...
void CCustomTitleUI::zoom_moving(int move)
{
...
void CCustomTitleUI::initialize()
{
	...
	std::string strBtnId[eBTN_MAX] = { "btn_make", "btn_cancel", "btn_close"
		, "btn_color_left", "btn_color_right", "btn_Backcolor_left", "btn_Backcolor_right", "btn_Effect_left", "btn_Effect_right", "tb_plus", "tb_minus", "tb_left", "tb_right" };
	...
	if (m_pbtn[eBTN_MODEL_PLUS] != NULL)
	{
		CmdCustomTitlePlus* pCmd = new CmdCustomTitlePlus;
		pCmd->setData(this);
		m_pbtn[eBTN_MODEL_PLUS]->SetCommandUp(pCmd);
	}

	if (m_pbtn[eBTN_MODEL_MINUS] != NULL)
	{
		CmdCustomTitleMinus* pCmd = new CmdCustomTitleMinus;
		pCmd->setData(this);
		m_pbtn[eBTN_MODEL_MINUS]->SetCommandUp(pCmd);
	}

	if (m_pbtn[eBTN_MODEL_LEFT] != NULL)
	{
		CmdCustomTitleMoving* pCmd = new CmdCustomTitleMoving;
		pCmd->setData(this, -25);
		m_pbtn[eBTN_MODEL_LEFT]->SetCommandUp(pCmd);
	}

	if (m_pbtn[eBTN_MODEL_RIGHT] != NULL)
	{
		CmdCustomTitleMoving* pCmd = new CmdCustomTitleMoving;
		pCmd->setData(this, 25);
		m_pbtn[eBTN_MODEL_RIGHT]->SetCommandUp(pCmd);
	}
	...
	if (m_pTbGuide != NULL)
	{
		CTString strDesc = UIMGR()->GetString(8173) + "\\n" + UIMGR()->GetString(8174) + "\\n"
			+ UIMGR()->GetString(8175) + "\\n" + UIMGR()->GetString(8176);
...
void CCustomTitleUI::OnRender(CDrawPort* pDraw)
{
	...
	if (m_ModelRenderTarget != NULL)
	{
		...
		((CPlayerEntity*)CEntity::GetPlayerEntity(0))->HUD_DrawModel(pDraw, 0, 0, _PreviewX, _PreviewY, HUD_CHARACTER_TYPE, m_ModelX, m_ModelY, m_ModelR);
...
void CCustomTitleUI::PressMakeBtn()
{
	...
	m_nMsgID = MSGBOXMGR()->CreateMsgBox(_S(8250, "호칭 제작"),
		_S(8251, "설정 하신 내용으로 호칭을 제작 합니다. 확인 클릭 시 인벤토리 호칭이 생성 됩니다."),
