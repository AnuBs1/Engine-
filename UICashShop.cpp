...
const INDEX _RankCount = 5; // ÃßÃµ ¾ÆÀÌÅÛ ·©Å© °¹¼ö
const INDEX _GoodsListCount = 8; // »óÇ° ¸ñ·Ï Ç¥½Ã °¹¼ö(ÆäÀÌÁö´ç)
const INDEX _PurchaseListCount = 25;
const INDEX _PurchaseGetCount = 10; // ±¸¸ÅÇÑ »óÇ° Áß ÇÑ¹ø¿¡ ÀÎº¥À¸·Î °¡Á®¿Ã ¼ö ÀÖ´Â ÃÖ´ë °¹¼ö
extern INDEX g_iCountry;

const INDEX _PreviewX = 140, _PreviewY = 226, _RTWidth = 366, _RTHeight = 226; // Ä³¸¯ÅÍ¸¦ Ç¥ÇöÇÏ´Â Preivew Screen À§Ä¡
...
//276,69
	...
	m_btnConfirm.Create(this, _S(2380, "°áÁ¦"), 0,0,108,21);
	...
	m_btnCancel.Create(this, _S(139, "Ãë¼Ò"), 0,0,108,21);
	...
	m_btnClose.Create(this, _s(""), 0,0,22,22);
	m_btnClose.SetUV(UBS_IDLE, 80,258,102,279, fTexWidth, fTexHeight);
	m_btnClose.SetUV(UBS_ON, 109,258,131,279, fTexWidth, fTexHeight);
	m_btnClose.SetUV(UBS_CLICK, 137,258,159,279, fTexWidth, fTexHeight);
	m_btnClose.CopyUV(UBS_IDLE, UBS_DISABLE);
	...
	case CSP_Package:
		{
			...
			m_btnCancel.SetText(_S(870, "´Ý±â"));
	...
	case CSP_Gift:
		{
			...
			m_btnConfirm.SetText(_S(3090, "º¸³»±â"));
	...
	case CSP_Purchase:
		{
			...
			m_btnConfirm.SetText(_S(2380, "°áÁ¦"));
	...
	case CSP_Package:
		{
			...
			pDrawPort->PutTextExCX(_S(5307, "ÆÐÅ°Áö Á¤º¸"), m_nPosX+150, m_nPosY+4, 0xFFFFFFFF);
	...
	case CSP_Gift:
		{
			...
			pDrawPort->PutTextExCX(_S(3104, "¼±¹° ¹ÞÀ» Ä£±¸"), m_nPosX+150, m_nPosY+4, 0xFFFFFFFF);
			pDrawPort->PutTextExRX(_S(5214, "Ä³¸¯ÅÍ ¸í"), m_nPosX+142,m_nPosY+28,0xFFFFFFFF);
			pDrawPort->PutTextExRX(_S(5215, "Ä£±¸¸ñ·Ï"), m_nPosX+142,m_nPosY+50,0xFFFFFFFF);
			pDrawPort->PutTextEx(_S(3101, "¼±¹°¸Þ½ÃÁö"), m_nPosX+17,m_nPosY+79,0xFFFFFFFF);
			...
			// 100 byteÁ¦ÇÑÀ¸·Î ÀÎÇØ MultiEditBox¿¡¼­ IME¿Ï¼ºÀÌ ¾ÈµÇ¼­ Á¦ÇÑ ¾ø¾Ú(ÀÔ·Â ¶óÀÎ Á¦ÇÑÀ¸·Î º¯°æ)
			CTString temp;
			//temp.PrintF("(%d/100 Byte)", m_edGiftContent.GetAllStringLength());
			temp = _S(5317, "(ÀÔ·Â 3ÁÙ Á¦ÇÑ)");
	...
	case CSP_Purchase:
		{
			...
			pDrawPort->PutTextExCX(_S(5304, "±¸¸Å °áÁ¦ Ã¢"), m_nPosX+150, m_nPosY+4, 0xFFFFFFFF);
			pDrawPort->PutTextExCX(_S(2392, "±¸¸ÅÇÑ ¹°Ç°"), m_nPosX+103,m_nPosY+40,0xFFFFFFFF);
			pDrawPort->PutTextExCX(_S(2397, "°¡°Ý"), m_nPosX+252,m_nPosY+40,0xFFFFFFFF);
			...
			pDrawPort->PutTextEx(_S(5216, "ÃÑ ±¸ÀÔ ±Ý¾×"), m_nPosX+20,m_nPosY+279, 0xFFFFFFFF);
	...
	case WM_LBUTTONDBLCLK:
		{
			...
			for (int i=0; i<5; ++i)
			{
				...
				// Àåºñ ¾ÆÀÌÅÛÀÌ¸é ´õºíÅ¬¸¯À¸·Î ÀÔ¾î º¼ ¼ö ÀÖ´Ù.
...
//	m_ModelRenderTarget->Init(_PreviewX,_PreviewY);
	...
	m_SelectedCount = 0; // ÃÖ´ë _PurchaseGetCount ±îÁö
	m_SelectedGoods = -1;
...
{
	/*if( m_nPosX < pixMinI || m_nPosX + GetWidth() > pixMaxI ||
		m_nPosY < pixMinJ || m_nPosY + GetHeight() > pixMaxJ )*/ // CashShop UIÀÇ °æ¿ì, ¹«Á¶°Ç ¼¾ÅÍ·Î °¡Á®°¡¾ß ÇÑ´Ù.
		ResetPosition( pixMinI, pixMinJ, pixMaxI, pixMaxJ );
...
void CUICashShopEX::InitCatalogNameNodes(void)
{
	...
	if (!bFileLoaded)
	{
		...
		m_CatalogNamesNodes[CSC_HOTandNew].AddNode(_S(5236, "½Å»óÇ°"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_HOTandNew].AddNode(_S(5237, "Æ¯°¡"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_HOTandNew].AddNode(_S(5238, "ÀÎ±â»óÇ°"), CCashTypeNode::CTN_FIRST);

		m_CatalogNamesNodes[CSC_Platinum].Create(NULL, _S(5230, "ÇÃ·¡Æ¼´½"), CCashTypeNode::CTN_CATALOG);

		m_CatalogNamesNodes[CSC_SpendGoods].Create(NULL, _S(5231, "¼Òºñ»óÇ°"), CCashTypeNode::CTN_CATALOG);
		m_CatalogNamesNodes[CSC_SpendGoods].AddNode(_S(5239, "Ä³¸¯ÅÍ ¼ºÀå"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5248, "°æÇèÄ¡"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5249, "¼÷·Ãµµ"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5250, "È®·ü"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5251, "È¥ÇÕ"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_SpendGoods].AddNode(_S(5240, "´É·ÂÄ¡ °­È­"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5252, "HP È®Àå"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5253, "MP È®Àå"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5254, "°ø°Ý·Â"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_SpendGoods].AddNode(_S(5241, "¹°¾à"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5255, "HP È¸º¹"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5256, "MP È¸º¹"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_SpendGoods].GetSubNode(iCount).AddNode(_S(5257, "µà¾ó È¸º¹"), CCashTypeNode::CTN_SECOND); iCount = 1;

		m_CatalogNamesNodes[CSC_EquipGoods].Create(NULL, _S(5232, "Àåºñ»óÇ°"), CCashTypeNode::CTN_CATALOG);
		m_CatalogNamesNodes[CSC_EquipGoods].AddNode(_S(5242, "Àåºñ"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(5258, "¹«±â"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(5259, "°©¿Ê"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(5274, "¾×¼¼¼­¸®"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_EquipGoods].AddNode(_S(5243, "Àåºñ °­È­"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(5261, "°áÇÕÁÖ¹®¼­"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(5262, "¼ÒÄÏ"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(5263, "ºí·¯µå"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(5275, "Á¦·Ã"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_EquipGoods].AddNode(_S(5260, "ÄÚ½ºÆ¬"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(43, "Å¸ÀÌÅº"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(44, "±â»ç"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(45, "Èú·¯"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(46, "¸ÞÀÌÁö"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(47, "·Î±×"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(48, "¼Ò¼­·¯"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_EquipGoods].GetSubNode(iCount).AddNode(_S(4410, "³ªÀÌÆ®¼Îµµ¿ì"), CCashTypeNode::CTN_SECOND); iCount = 1;

		m_CatalogNamesNodes[CSC_Avata].Create(NULL, _S(5233, "¾Æ¹ÙÅ¸"), CCashTypeNode::CTN_CATALOG);
		m_CatalogNamesNodes[CSC_Avata].AddNode(_S(5244, "Æê »óÇ°"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_Avata].GetSubNode(iCount).AddNode(_S(5264, "Æê"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_Avata].GetSubNode(iCount).AddNode(_S(5265, "Æê ¾ÆÀÌÅÛ"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_Avata].AddNode(_S(5245, "¿ëº´"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_Avata].GetSubNode(iCount).AddNode(_S(5266, "¸ó½ºÅÍ ¿ëº´"), CCashTypeNode::CTN_SECOND); iCount = 1;

		m_CatalogNamesNodes[CSC_ServiceGoods].Create(NULL, _S(5234, "¼­ºñ½º»óÇ°"), CCashTypeNode::CTN_CATALOG);
		m_CatalogNamesNodes[CSC_ServiceGoods].AddNode(_S(5246, "ÆíÀÇ »óÇ°"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_ServiceGoods].GetSubNode(iCount).AddNode(_S(5267, "ÁÖ¹®¼­"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_ServiceGoods].GetSubNode(iCount).AddNode(_S(5268, "Ä«µå"), CCashTypeNode::CTN_SECOND); iCount = 1;
		m_CatalogNamesNodes[CSC_ServiceGoods].AddNode(_S(5247, "±âÅ¸"), CCashTypeNode::CTN_FIRST);

		m_CatalogNamesNodes[CSC_PackageGoods].Create(NULL, _S(5235, "ÆÐÅ°Áö»óÇ°"), CCashTypeNode::CTN_CATALOG);
		m_CatalogNamesNodes[CSC_PackageGoods].AddNode(_S(5239, "Ä³¸¯ÅÍ ¼ºÀå"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5248, "°æÇèÄ¡"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5249, "¼÷·Ãµµ"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5250, "È®·ü"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5251, "È¥ÇÕ"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_PackageGoods].AddNode(_S(5240, "´É·ÂÄ¡ °­È­"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5252, "HP È®Àå"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5253, "MP È®Àå"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_PackageGoods].AddNode(_S(5241, "¹°¾à"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5255, "HP È¸º¹"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5256, "MP È¸º¹"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5257, "µà¾ó È¸º¹"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_PackageGoods].AddNode(_S(5260, "ÄÚ½ºÆ¬"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(43, "Å¸ÀÌÅº"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(44, "±â»ç"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(45, "Èú·¯"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(46, "¸ÞÀÌÁö"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(47, "·Î±×"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(48, "¼Ò¼­·¯"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(4410, "³ªÀÌÆ®¼Îµµ¿ì"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_PackageGoods].AddNode(_S(5243, "Àåºñ °­È­"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5261, "°áÇÕÁÖ¹®¼­"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5275, "Á¦·Ã"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5262, "¼ÒÄÏ"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_PackageGoods].AddNode(_S(5244, "Æê »óÇ°"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5264, "Æê"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5265, "Æê ¾ÆÀÌÅÛ"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_PackageGoods].AddNode(_S(5245, "¿ëº´"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_PackageGoods].AddNode(_S(5246, "ÆíÀÇ »óÇ°"), CCashTypeNode::CTN_FIRST);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5267, "ÁÖ¹®¼­"), CCashTypeNode::CTN_SECOND);
		m_CatalogNamesNodes[CSC_PackageGoods].GetSubNode(iCount).AddNode(_S(5268, "Ä«µå"), CCashTypeNode::CTN_SECOND); iCount++;
		m_CatalogNamesNodes[CSC_PackageGoods].AddNode(_S(5247, "±âÅ¸"), CCashTypeNode::CTN_FIRST);
	}
}
...
void CUICashShopEX::OpenCashShopEX()
{
	...
	pHud->HUD_SetModelData(HUD_CHARACTER_TYPE, -1, eRENDER_UI_TYPE_CASHSHOP, 999);
...
// 						}
			...
			m_bPackageInfo = FALSE; // ÆÐÅ°Áö ¹öÆ°¿¡ ¸¶¿ì½º ¿À¹ö½Ã µ¸º¸±â Ä¿¼­·Î º¯°æ
	...
	case WM_LBUTTONUP:
		{
			...
			if (pUIManager->GetDragIcon() == NULL)
			{
				...
				{ // ÅÇÇüÅÂÀÌ±â ¶§¹®¿¡ ±»ÀÌ Button Up Ã³¸®°¡ ÇÊ¿äÇÏÁö ¾ÊÀ» µí ÇÔ.
				}
				else if (IsInsideRect(nX, nY, m_CashShopFrame[CSF_Category]))
				{
					if (m_BtnSearch.MouseMessage(pMsg) != WMSG_FAIL)
				...
				else if (IsInsideRect(nX, nY, m_CashShopFrame[CSF_View]))
				{
					...
					else if (m_ViewFrameType == VFT_PurchaseList)
					{
						...
						if (m_BtnMove.MouseMessage(pMsg) != WMSG_FAIL) // ¸ðµÎ ¿Å±â±â
						{
							MovePurchaseGoods();
							return WMSG_SUCCESS;
						}
					...
					for (i=0; i<BIT_Total; ++i)
					{
						if (m_btnInven[i].MouseMessage(pMsg) != WMSG_FAIL)
						{
							...
							case BIT_Clear: // Àå¹Ù±¸´Ï ºñ¿ì±â
								...
								break;
							case BIT_PurchaseItem: // ¾ÆÀÌÅÛ ±¸¸ÅÇÏ±â
								{
									m_Popup.ClearGoodsData();
									for (i=0; i<MAX_CASHINVEN; ++i)
									{
							...
							case BIT_CashCharge: // Ä³½¬ ÃæÀü
								{
...
#elif defined(G_RUSSIA )
									{
										...
										ShellExecute( NULL, "open", "http://lcgame.su/account/", NULL,NULL, SW_SHOWNORMAL );										
									}
									// sam -->|
...
#else
									{
										...
										CTString strCallUrl;
#if defined G_KOR // ±¹³»´Â À¥ °áÁ¦ ÆäÀÌÁö¸¦ È£Ãâ½Ã, ÆÄÆ®³Ê ÄÚµå¸¦ ÆÄ¶ó¹ÌÅÍ·Î º¸³½´Ù.
										extern BOOL g_bAutoLogin;
										if (g_bAutoLogin)
										{
											strCallUrl.PrintF(_S(5318, ""), _pNetwork->m_strUserCID);
										}
										else
										{
											strCallUrl.PrintF(_S(5318, ""), CTString("or"));
										}
						...
						{ // Àåºñ ¾ÆÀÌÅÛ¸¸ µî·ÏÀÌ µÇ¾î¾ß ÇÑ´Ù.
							if (m_ViewFrameType == VFT_GoodsList || m_ViewFrameType == VFT_WishList)
							{
								...
								if (tmpData != NULL)
								{
									...
									{ // UniIndex¿¡ ÇÑ¹úÀÇ»ó ¾ÆÀÌÅÛ Index°¡ µî·ÏµÇ¾ú´Ù.
	...
	case VFT_GoodsList:
		{
			...
			for (i=0; i<8; ++i)
			{
				...
				{
					if (m_GoodsBoxList[i].GoodsData->IsDiscount()) // Æ¯°¡ »óÇ°
					{
						m_GoodsBoxList[i].BoxSurfaceSell.SetPos(m_nPosX, m_nPosY);
						m_GoodsBoxList[i].BoxSurfaceSell.RenderRectSurface(pDrawPort, 0xFFFFFFFF);
					}
					else {
						m_GoodsBoxList[i].BoxSurface.SetPos(m_nPosX, m_nPosY);
						m_GoodsBoxList[i].BoxSurface.RenderRectSurface(pDrawPort, 0xFFFFFFFF);
					...
					else
					{
						if (m_GoodsBoxList[i].GoodsData->m_itemListCnt > 0)
						{
							...
							pDrawPort->PutTextEx(strCount, m_nPosX+rtTmpBox.Left+45, m_nPosY+rtTmpBox.Top+57, 0xFFFFFFFF); // ¾ÆÀÌÅÛ °¹¼ö
						}
					}

					if (m_GoodsBoxList[i].GoodsData->IsNew()) // ½Å»óÇ°
					{
					...
					if (m_GoodsBoxList[i].GoodsData->IsPopular()) // ÀÎ±â »óÇ°
					{
						pDrawPort->AddTexture(nTmpX, nTmpY, nTmpX+22, nTmpY+11,
				...
				/*	if (m_GoodsBoxList[i].GoodsData->IsDiscount()) // Æ¯°¡ »óÇ°
					...
					if(m_GoodsBoxList[i].GoodsData->CannotGift())
					pDrawPort->PutTextEx(m_GoodsBoxList[i].GoodsData->m_itemName, 
					m_nPosX + rtTmpBox.Left + 21, m_nPosY + rtTmpBox.Top + 10, 0xFF2100FF); // ¾ÆÀÌÅÛ ÀÌ¸§
					else 
						pDrawPort->PutTextEx(m_GoodsBoxList[i].GoodsData->m_itemName,
						m_nPosX + rtTmpBox.Left + 21, m_nPosY + rtTmpBox.Top + 10, 0xFFFFFFFF); // ¾ÆÀÌÅÛ ÀÌ¸§
					...
					strPrice.PrintF(_S(255, "°¡°Ý: %s"), strCash);

					if (m_GoodsBoxList[i].GoodsData->m_limitCnt > 0)
					{
						strRemain.PrintF(_S(2890, "³²Àº ¼ö·®: %d"), m_GoodsBoxList[i].GoodsData->m_limitCnt);
					}
					else if (m_GoodsBoxList[i].GoodsData->m_limitCnt == 0)
					{
						strRemain = _S(2891, "³²Àº ¼ö·® : Ç°Àý");
					}

					pDrawPort->PutTextExRX(strPrice, m_nPosX+rtTmpBox.Left+203, m_nPosY+rtTmpBox.Top+41, 0xFFFFFFFF); // »óÇ° °¡°Ý
					pDrawPort->PutTextExRX(strRemain, m_nPosX+rtTmpBox.Left+203, m_nPosY+rtTmpBox.Top+54, 0xFFFFFFFF); // ³²Àº ¼ö·®
				}
			}
	...
	case VFT_PurchaseList:
		{
			...
			m_BtnGift.Render();
//345
			pDrawPort->PutTextExCX(_S(2392, "±¸¸ÅÇÑ ¹°Ç°"), m_nPosX+124, m_nPosY+218, 0xFFFFFFFF);
			pDrawPort->PutTextExCX(_S(2393, "¿Å±æ ¹°Ç°"), m_nPosX+345, m_nPosY+218, 0xFFFFFFFF);
	...
	case VFT_GiftList:
		{
			...
			m_BtnGet.Render();
			m_GiftScroll.Render();
			pDrawPort->PutTextExCX(_S(3098, "¹ÞÀº ¼±¹°ÇÔ"), m_nPosX+124, m_nPosY+218, 0xFFFFFFFF);
			pDrawPort->PutTextExCX(_S(3101, "¼±¹° ¸Þ½ÃÁö"), m_nPosX+345, m_nPosY+218, 0xFFFFFFFF);
	...
	case VFT_WishList:
		{
			...
			for (i=0; i<8; ++i)
			{
				...
				else
				{
					...
					if (m_WishBoxList[i].GoodsData->IsNew()) // ½Å»óÇ°
					{
						pDrawPort->AddTexture(nTmpX, nTmpY, nTmpX+22, nTmpY+11,
							m_uvFlag[CFI_New].U0, m_uvFlag[CFI_New].V0, m_uvFlag[CFI_New].U1, m_uvFlag[CFI_New].V1, 0xFFFFFFFF);
						nTmpX += (22+2);
					...
					if (m_WishBoxList[i].GoodsData->IsPopular()) // ÀÎ±â »óÇ°
					{
						pDrawPort->AddTexture(nTmpX, nTmpY, nTmpX+22, nTmpY+11,
							m_uvFlag[CFI_Popular].U0, m_uvFlag[CFI_Popular].V0, m_uvFlag[CFI_Popular].U1, m_uvFlag[CFI_Popular].V1, 0xFFFFFFFF);
						nTmpX += (22+2);
					...
					if (m_WishBoxList[i].GoodsData->IsDiscount()) // Æ¯°¡ »óÇ°
					{
						pDrawPort->AddTexture(nTmpX, nTmpY, nTmpX+22, nTmpY+11,
							m_uvFlag[CFI_Discount].U0, m_uvFlag[CFI_Discount].V0, m_uvFlag[CFI_Discount].U1, m_uvFlag[CFI_Discount].V1, 0xFFFFFFFF);
						nTmpX += (22+2);
					...
					pUIManager->InsertCommaToString(strCash);
					strPrice.PrintF(_S(255, "°¡°Ý: %s"), strCash);
					strRemain.PrintF(_S(2890, "³²Àº ¼ö·®: %d"), m_WishBoxList[i].GoodsData->m_limitCnt);
					pDrawPort->PutTextExRX(strPrice, m_nPosX+rtTmpBox.Left+203, m_nPosY+rtTmpBox.Top+46, 0xFFFFFFFF); // »óÇ° °¡°Ý
				}
			}
...
///	CashShop Frame Preview begin() ///////////////////////////////////////////////////////////////////////////////////////
	...
	if (pUIManager->GetRenderTarget() != NULL)
	{
		...
		((CPlayerEntity*)CEntity::GetPlayerEntity(0))->HUD_DrawModel(pDrawPort, 0, 0, _PreviewX, _PreviewY, HUD_CHARACTER_TYPE, -3.2f, -1.0f, 999);
...
/// CashShop Frame Iventory begin() //////////////////////////////////////////////////////////////////////////////////////
	pDrawPort->PutTextExRX(_S(2389, "»óÇ° ±Ý¾×"),  m_nPosX+630,m_nPosY+392, 0xFFFFFFFF);
	pDrawPort->PutTextExRX(_S(2390, "º¸À¯ Ä³½¬"), m_nPosX+630,m_nPosY+418, 0xFFFFFFFF);
...
void CUICashShopEX::BasicBackgroundInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	m_btnClose.Create(this, CTString(""), 728,1,22,22);
	m_btnClose.SetUV(UBS_IDLE, 80,258,102,280, fTexWidth, fTexHeight);
	m_btnClose.SetUV(UBS_ON, 109,258,131,280, fTexWidth, fTexHeight);
	m_btnClose.SetUV(UBS_CLICK, 137,258,159,280, fTexWidth, fTexHeight);
	m_btnClose.CopyUV(UBS_IDLE, UBS_DISABLE);
...
void CUICashShopEX::MainTabFrameInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	m_BtnMainTab[CS_IrisShop].Create(this,_S(2572, "¾ÆÀÌ¸®½º »óÁ¡"), 7,36,113,22);
	m_BtnMainTab[CS_PurchaseInfo].Create(this,_S(2386, "±¸¸Å Á¤º¸"), 121,36,113,22);
	m_BtnMainTab[CS_GotTheGift].Create(this,_S(3088, "¹ÞÀº ¼±¹°"), 235,36,113,22);
	m_BtnMainTab[CS_WishPage].Create(this, _S(5218, "¼Ò¸Á»óÀÚ"), 349,36,113,22);
...
void CUICashShopEX::CategoryFrameInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	m_Background.AddRectSurface(UIRect(16,141,133,162), UIRectUV(39,236,157,256,fTexWidth,fTexHeight));
	...
	m_cbCategoryType[CST_First].Create(this, 174,142,118,20,102,4,13,13,10, _pUIFontTexMgr->GetFontHeight()+5, 0, 0);
	...
	m_cbCategoryType[CST_Second].Create(this, 294,142,118,20,102,4,13,13,10, _pUIFontTexMgr->GetFontHeight()+5, 0, 0);
...
void CUICashShopEX::ViewFrameInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	// ¼Ò¸Á »óÀÚ ¸®½ºÆ®µµ µðÀÚÀÎÀÌ °°À¸¹Ç·Î ¿©±â¼­ ¼ÂÆÃÇÑ´Ù.
	// ÀÌ°ÍÀº °¡·Î ¹è¿­
	for (i=0; i<4; ++i)
	{
		UIRect rtTempBox = rtBox;
		...
		for (j=0; j<2; ++j)
		{
			...
			m_GoodsBoxList[(i * 2) + j].BoxSurfaceSell.AddRectSurface(rtTempBox, UIRectUV(0, 519, 213, 593, fTexWidth, fTexHeight));
	...
	m_BtnMove.Create(this, _S(5219, "¸ðµÎ ¿Å±â±â"), 71,463,96,21);
	...
	m_BtnGet.Create(this, _S(2383, "°¡Á®¿À±â"), 299,363,96,21);
	...
	m_BtnGift.Create(this, _S(3089, "¼±¹°ÇÏ±â"), 299,394,96,21);
...
void CUICashShopEX::PreviewFrameInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	m_btnRestore.Create(this, _S(5220, "¿ø·¡´ë·Î µ¹¸®±â"), 603,268,129,21);
...
void CUICashShopEX::InvenFrameInit(FLOAT fTexWidth, FLOAT fTexHeight)
{
	...
	m_btnInven[BIT_Clear].Create(this, _S(5221, "Àå¹Ù±¸´Ï ºñ¿ì±â"), 564,439,164,21);
	m_btnInven[BIT_PurchaseItem].Create(this, _S(2385, "¾ÆÀÌÅÛ ±¸¸Å"), 564,464,164,21);
	m_btnInven[BIT_CashCharge].Create(this, _S(2387, "Ä³½¬ ÃæÀü"), 564,488,164,21);
	m_btnInven[BIT_Exit].Create(this, _S(4117, "Á¾·á"), 564,511,164,21);
...
{
	ULONG ulSearchType; // °Ë»ö Á¾·ù
	UBYTE ubCatalogCount; // »óÇ° °¹¼ö 0 ~ 8
	ULONG ulCTID; // »óÇ° °íÀ¯ »öÀÎ
	SLONG slRemainCount; // ÇÑÁ¤ »óÇ° ³²Àº °¹¼ö
	ULONG ulPrice; // »óÇ° °¡°Ý
	ULONG ulCurrentPage; // ÇöÀç ÆäÀÌÁö
	ULONG ulEndPage; // ÀüÃ¼ ÆäÀÌÁö ¼ö

	(*istr) >> ulSearchType >> ubCatalogCount;

	m_CurrentPageType = ulSearchType;
	ClearGoodsList(); // ÆäÀÌÁö »óÇ° Á¤º¸ Å¬¸®¾î
...
void CUICashShopEX::ClearCashType(void)
{
	...
	for (i=0; i<CSC_Total; ++i)
	{
		...
		if (i==0)
		{
			CS_FirstSubType& tmpCTTypeInfo1 = m_CashSubTypeInfo[i].FirstSubTypeIndexList.Push(); // ½Å»óÇ°
			tmpCTTypeInfo1.iFirstSubType = 1;
			CS_FirstSubType& tmpCTTypeInfo2 = m_CashSubTypeInfo[i].FirstSubTypeIndexList.Push(); // Æ¯°¡
			tmpCTTypeInfo2.iFirstSubType = 2;
			CS_FirstSubType& tmpCTTypeInfo3 = m_CashSubTypeInfo[i].FirstSubTypeIndexList.Push(); // ÀÎ±â»óÇ°
...
void CUICashShopEX::SetCashType(CNetworkMessage *istr)
{
	...
	// Category´Â °íÁ¤ÀÌ¹Ç·Î ³ªÁß¿¡ »ý°¢ÇÏÀÚ.
	// Category type 
	//(*istr) >> ulMax;

	ClearCashType(); // ÃÊ±âÈ­ ÇÑ´Ù.
	...
	for (i=0; i<ulSubTypeCount; ++i)
	{
		...
		//ASSERT(iFirstSubType > 1 && "is not first subtype"); // 1Àº ¼­¹ö¿¡¼­ none typeÀ¸·Î Á¤ÀÇ µÇ¾î ÀÖ´Ù.
		if (iFirstSubType == 0) continue; // none typeÀº °¡ÁöÁö ¾Ê´Â´Ù.
	...
	for (i=0; i<ulSubTypeCount; ++i)
	{
		...
		if (iSecondSubType == 0) continue; // none typeÀº °¡ÁöÁö ¾Ê´Â´Ù.
		//ASSERT(iSecondSubType > 1 && "is not second subtype"); // 1Àº ¼­¹ö¿¡¼­ none typeÀ¸·Î Á¤ÀÇ µÇ¾î ÀÖ´Ù.
...
void CUICashShopEX::SetRClickMenu(INDEX iItem)
{
	...
	m_tpCommand.AddMenuList(_S(5224, "¾ÆÀÌÅÛ ±¸¸ÅÇÏ±â"), 0xc2bac5FF, CSC_ItemPurchase);
	if (tmpData->m_itemListCnt == 1) // ÆÐÅ°Áö »óÇ°Àº ÆÐÅ°Áö »óÇ° ¾È¿¡¼­ ÀÔ´Â´Ù.
	{
		...
		{ // ÇÑ¹ú ÀÇ»ó°ú ³¯°³µµ ÀÔÀ» ¼ö ¾ø´Ù.
			m_tpCommand.AddMenuList(_S(5225, "ÄÚ½ºÆ¬ ÀÔ±â"), 0xc2bac5FF, CSC_CostumeWear);
		}
	}

	m_tpCommand.AddMenuList(_S(5226, "Àå¹Ù±¸´Ï¿¡ ´ã±â"), 0xc2bac5FF, CSC_AddInven);
	if (m_ViewFrameType == VFT_GoodsList)
	{
		m_tpCommand.AddMenuList(_S(5227, "¼Ò¸Á »óÀÚ¿¡ ´ã±â"), 0xc2bac5FF, CSC_AddWishBox);
	}
	else if (m_ViewFrameType == VFT_WishList)
	{
		m_tpCommand.AddMenuList(_S(5228, "¼Ò¸Á »óÀÚ¿¡¼­ »èÁ¦"), 0xc2bac5FF, CSC_AddWishBox);
	}
	m_tpCommand.AddMenuList(_S(139, "Ãë¼Ò"), 0xc2bac5FF, CSC_Cancel);
	m_tpCommand.Show();
}
...
void CUICashShopEX::SetWearingModel(CUIIcon* srcBtn, INDEX iCashIndex)
{
	...
	{
		// Âø¿ëÇÒ ¼ö ÀÖ´Â Á÷¾÷±ºÀÌ ¾Æ´Ï°Å³ª ¸Þ½Ã º¯È­°¡ ¾ø´Â Çï¸äÀÓ
		return;
	}
	...
	if (pItemData->GetType() == CItemData::ITEM_WEAPON || pItemData->GetType() == CItemData::ITEM_SHIELD)
	{
		...
		else
		{
			...
			if (pItemData->GetSubType() == CItemData::ITEM_SHIELD_HEAD) {
				...
				iType = WEAR_SHIELD;
				m_iEquipCashIndex[6] = iItemIndex; // ¹æÆÐ¸¸ dbitemindex¸¦ ¾´´Ù.
			}
...
void CUICashShopEX::SetTakeOffModel(INDEX iRef)
{
	if (iRef > 0)
	{
		...
		{ // ¹«±â¸¦ ¹þÀ» ¶§´Â ¹æÆÐµµ ¹þ´Â´Ù.
			iRef = 0; // 0¹øÀº ¹«±â
			iType = WEAR_WEAPON; // ¹æÆÐ ¾ÆÀÌÅÛÀº ¹ö¸°´Ù.//iType = WEAR_SHIELD;
			((CPlayerEntity*)CEntity::GetPlayerEntity(0))->HUD_WearingModel(WEAR_SHIELD,_pNetwork->MyCharacterInfo.job,0,FALSE);
			m_iEquipCashIndex[6] = 0;
		}
...
void CUICashShopEX::SendWishItems(ULONG ulCTID, BOOL bCommit)
{
	if (ulCTID > 0)
	{
		...
		if (bCommit)
		{
			nmItem << (UBYTE)MSG_EX_CASHITEM_WISHLIST_SAVE_REQ; // µî·Ï
		}
		else
		{
			nmItem << (UBYTE)MSG_EX_CASHITEM_WISHLIST_DEL_REQ; // »èÁ¦			
		}
...
void CUICashShopEX::RevWishList(CNetworkMessage* istr, BOOL bCommit)
{
		...
		case 1:
			{
				if (bCommit)
				{
					Message(MSGCMD_CASH_EX_ERROR, _S(5270, "¼Ò¸Á»óÀÚ"), _S(5271, "´õ ÀÌ»ó ¼Ò¸Á »óÀÚ¿¡ ¾ÆÀÌÅÛÀ» ´ãÀ» ¼ö ¾ø½À´Ï´Ù."), UMBS_OK);
				}
				else
				{
					Message(MSGCMD_CASH_EX_ERROR, _S(5270, "¼Ò¸Á»óÀÚ"), _S(5272, "¼Ò¸Á »óÀÚ¿¡ Á¸ÀçÇÏÁö ¾Ê´Â »óÇ°ÀÔ´Ï´Ù."), UMBS_OK);
				}
			}
			break;
		case 2:
			{
				Message(MSGCMD_CASH_EX_ERROR, _S(5270, "¼Ò¸Á»óÀÚ"), _S(5273, "½Ã½ºÅÛ ¿À·ùÀÔ´Ï´Ù. ¿î¿µÀÚ¿¡°Ô ¹®ÀÇÇÏ½Ê½Ã¿À."), UMBS_OK);
			}
			break;
		}
	}
	else
	{
		if (bCommit)
		{
			Message(MSGCMD_CASH_EX_ERROR, _S(5270, "¼Ò¸Á»óÀÚ"), _S(5315, "¼Ò¸Á»óÀÚ ´ã±â¸¦ ¿Ï·áÇÏ¿´½À´Ï´Ù"), UMBS_OK);
		}
...
void CUICashShopEX::SendGetRevGiftItem(INDEX iPos)
{
	...
	nmItem << (ULONG)1; // ÇöÀç ¼±¹° °¡Á®¿À±â´Â ÇÏ³ª¾¿¸¸ °¡Á®¿Â´Ù.
...
void CUICashShopEX::SetSuitWearingModel(INDEX iItem, INDEX iCash)
{
	...
	for (int i=0; i<5; ++i)
	{
		...
		// UniIndex¿¡ ÇÑ¹úÀÇ»ó ¾ÆÀÌÅÛ ÀÎµ¦½º¸¦ µî·Ï
...
void CUICashShopEX::StartSound()
{
	...
	//penPlayerEntity->PlayOpenCatalogSound();
}
